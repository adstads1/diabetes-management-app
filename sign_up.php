...
//Datepicker with day, month and year format
    ...
    var instances = M.Datepicker.init(elems, {format:'yyyy:mm:dd', yearRange:[1950,2020]});
  });
...
$specialChars = preg_match('@[^\w]@', $pword);
		...
		if(!empty($_POST['user']) && !empty($_POST['fname']) && !empty($_POST['sname']) && !empty($_POST['pword']) && !empty($_POST['dob']) && !empty($_POST['gender']) 
		   && !empty($_POST['weight'])) {  
			//Password validation.
